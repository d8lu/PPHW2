...
bool Image::save(std::string file_path) {
    ...
    bool success = stbi_write_jpg(file_path.c_str(), width, height, channels,
                                  out_data, 100);
...
void Image::set_pixel(int x, int y, int c, float val) {
    // if (x >= width || x < 0 || y >= height || y < 0 || c >= channels || c <
    // 0) {
...
Image gaussian_blur(const Image& img, float sigma) {
    ...
    // convolve vertical
    // convolve vertical (Optimized)
    // We iterate column by column
    #pragma omp parallel for schedule(static)

    for (int x = 0; x < img.width; x++) {
        // 1. Handle the top border (y < center)
        // Here, the kernel window extends above the image (y+dy < 0), so we
        // must use the safe get_pixel() for clamping.
        ...
        // 2. Handle the fast inner part (center <= y < height - center)
        // In this region, the entire kernel is guaranteed to be within the
        // image bounds. We can use direct pointer access and avoid all checks.
        for (int y = center; y < img.height - center; y++) {
            // Get a pointer to the source pixel at the top of the kernel window
            float* p_src = &img.data[(y - center) * img.width + x];

            float sum = 0;
            for (int k = 0; k < size; k++) {
                // Read value from the source pointer and multiply with kernel
                sum += (*p_src) * kernel.data[k];

                // Move the source pointer down by one row (i.e., by 'width'
                // elements)
                p_src += img.width;
            }

            // Write the result directly to the destination data array
            tmp.data[y * img.width + x] = sum;
        }

        // 3. Handle the bottom border (y >= height - center)
        // Here, the kernel window extends below the image, so we again use the
        // safe get_pixel().
        for (int y = img.height - center; y < img.height; y++) {
            float sum = 0;
            for (int k = 0; k < size; k++) {
                int dy = -center + k;
                sum += img.get_pixel(x, y + dy, 0) * kernel.data[k];
    ...
    // convolve horizontal
    // convolve horizontal (Optimized)
    #pragma omp parallel for schedule(static)

    for (int y = 0; y < img.height; y++) {
        // 1. Handle the left boundary where clamping is needed
        for (int x = 0; x < center; x++) {
            float sum = 0;
            for (int k = 0; k < size; k++) {
                int dx = -center + k;
                sum += tmp.get_pixel(x + dx, y, 0) *
                       kernel.data[k];  // Use safe get_pixel here
            }
            filtered.set_pixel(x, y, 0, sum);
        }

        // 2. Handle the fast inner part with direct pointer access
        // For a 1-channel image, data layout is [y*width + x]
        for (int x = center; x < img.width - center; x++) {
            float sum = 0;
            // Get a pointer to the start of the window in the source image
            float* p = &tmp.data[y * img.width + x - center];
            for (int k = 0; k < size; k++) {
                // No boundary checks, no complex index math, just pointer
                // increments
                sum += p[k] * kernel.data[k];
            }
            // Direct access for writing too
            filtered.data[y * img.width + x] = sum;
        }

        // 3. Handle the right boundary where clamping is needed
        for (int x = img.width - center; x < img.width; x++) {
            float sum = 0;
            for (int k = 0; k < size; k++) {
                int dx = -center + k;
                sum += tmp.get_pixel(x + dx, y, 0) *
